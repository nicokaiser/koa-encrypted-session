...
module.exports = function encryptedSession(opts, app) {
  ...
  if (opts.secret) {
    if (Buffer.byteLength(opts.secret) < 32) {
      throw new Error(
        'Invalid key length: opts.secret must be at least 32 bytes'
      );
    }
    ...
    if (opts.salt)
      salt = Buffer.isBuffer(opts.salt)
        ? opts.salt
        : Buffer.from(opts.salt, 'ascii');
  ...
  if (opts.secretKey) {
    ...
    if (secretKey.length < sodium.crypto_secretbox_KEYBYTES)
      throw new Error(
        `secretKey must be at least ${sodium.crypto_secretbox_KEYBYTES} bytes`
      );
  }
  ...
  opts.decode = (cookie) => {
    ...
    if (cipher.length < sodium.crypto_secretbox_MACBYTES)
      throw new SyntaxError('not long enough');

    const msg = Buffer.allocUnsafe(
      cipher.length - sodium.crypto_secretbox_MACBYTES
    );
    if (!sodium.crypto_secretbox_open_easy(msg, cipher, nonce, secretKey))
      throw new SyntaxError('unable to decrypt');
  ...
  opts.encode = (session) => {
    ...
    return encodeURIComponent(
      cipher.toString('base64') + ';' + nonce.toString('base64')
    );
  };
